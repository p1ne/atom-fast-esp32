...
public:
	BLEClient();
	BLEClient(BLEAdvertisedDevice* device);
	~BLEClient();
	...
	esp_ble_addr_type_t                        getPeerType();
	...
	bool                                       isDisconnected();              // Return true if we are disconnected.
